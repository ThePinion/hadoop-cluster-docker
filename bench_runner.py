...
def resize_cluster(cluster: int):
    ...
    result = subprocess.run(["./resize-cluster.sh", str(cluster)], stdout=subprocess.PIPE, text=True)
    print(f"Finished resizing cluster to {cluster} nodes!")
    

def run_benchmark(cluster: int, data_size: int, mapps: int, reduces: int) -> (int, int, int):
    print(f"Running benchmark with size {data_size} MB, mappers {mapps} and reducers {reduces}...")
    resize_cluster(cluster)
    reset_dir(BOOKS_TEMP_DIR)
    copy_files_until_size(BOOKS_DIR, BOOKS_TEMP_DIR, data_size)
    result = subprocess.run([SCRIPT_PATH, str(mapps), str(reduces)], stdout=subprocess.PIPE, text=True)
    print(f"Finished benchmark with size {data_size} MB, mappers {mapps} and reducers {reduces}!")
